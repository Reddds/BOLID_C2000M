...
// Интервал между последующими запросами мастера
...
// Действия при переполнении счётчика millis()
...
/**
 * обработка команд для мастера при каждой итерации осного цикла в main
 */
void MasterProcess()
{
    ...
    if(ModbusGetState() == COM_IDLE && nextModbusQuery < curMs)
    {
        ...
        //    {
                ...
                switch(ControllersNextRequest[longControllerId].banned)
                {
                    ...
                    break;
                    case CB_FIRST_BAN:
                        // Если интервал опроса больше интервала бана, то ставим интервал опроса
